...
public class SearchSystem
{
    ...
    private unsafe int NegaMax(NodeType nodeType, ref BoardData board, int depth, int alpha, int beta, int movesPlayed )
    {
        ...
        if (!isRoot && (MoveHistory.IsRepetition( board.ZobristKey ) || board.HalfMoves >= 100))
            return 0;
        ...
        if (!isRoot && entry.PositionKey == board.ZobristKey && entry.Depth >= depth)
        {
        ...
        for (int moveIndex = 0; moveIndex < selector.Length; moveIndex++)
        {
            ...
            // If we are either in a non pv node, or in a pv node where we have already checked the PV move, do a zero window search
            if (!isPV || localMoveCount > 0)
                newValue = -NegaMax(NodeType.NonPV, ref boardCopy, depth - 1, -alpha-1, -alpha, movesPlayed + 1);

            // If we are in a PV node and we didn't yet check the Pv move, or the zero window search hit alpha
    ...
    private unsafe int QuiesenceSearch(NodeType nodeType, ref BoardData board, int alpha, int beta)
    {
        ...
        //can't do standpat when in check
        if (!board.IsSideToMoveInCheck)
        {
            bestValue = EvaluationSystem.EvaluatePosition( ref board );
        }
